...
HAVING
	SUM(SALARY) = (
		SELECT   MAX(SUM(SALARY)) -- 서브쿼리 -> 21760000
		FROM     EMPLOYEE
		GROUP BY DEPT_CODE;
	);
...
SELECT 
	...
	J.JOB_CODE, D.DEPT_TITLE, J.JOB_NAME
FROM 
	EMPLOYEE E
JOIN 
	JOB J ON (E.JOB_CODE = J.JOB_CODE)
...
SELECT
	MAIN.EMP_ID , MAIN.EMP_NAME, MAIN.DEPT_CODE,
	NVL(D.DEPT_TITLE, '없음') AS DEPT_TITLE,
...
SELECT
	E.EMP_ID, EMP_NAME, E.PHONE, E.SALARY, J.JOB_NAME 
FROM
	EMPLOYEE E
JOIN 
	JOB J ON (E.JOB_CODE = J.JOB_CODE)
...
-- 3. 노옹철 사원과 같은 부서, 같은 직급인 사원을 조회하시오. 
--  (단, 노옹철 사원은 제외)
--  사번, 이름, 부서코드, 직급코드, 부서명, 직급명
SELECT
...
--    사번, 이름, 부서코드, 사수번호, 주민번호, 고용일     
SELECT
	EMP_ID, EMP_NAME, DEPT_CODE, JOB_CODE, EMP_NO, HIRE_DATE 
FROM 
	EMPLOYEE 
WHERE 
...
SELECT 
	E.EMP_ID, 
	E.EMP_NAME, 
	NVL(D.DEPT_TITLE,'소속없음') DETP_TITLE,
	JOB_NAME, 
	HIRE_DATE
FROM
	EMPLOYEE E
